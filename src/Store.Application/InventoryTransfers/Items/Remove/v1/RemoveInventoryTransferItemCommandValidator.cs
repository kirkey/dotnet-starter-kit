...
public class RemoveInventoryTransferItemCommandValidator : AbstractValidator<RemoveInventoryTransferItemCommand>
{
    public RemoveInventoryTransferItemCommandValidator([FromKeyedServices("store:inventory-transfers")] IReadRepository<InventoryTransfer> transferReadRepository)
    {
        ...
        RuleFor(x => x).MustAsync(async (cmd, ct) =>
        {
            ...
            if (transfer is null) return true; // previous rule handles this
