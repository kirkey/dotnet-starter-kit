...
public sealed class JwtAuthenticationService : AuthenticationStateProvider, IAuthenticationService, IAccessTokenProvider, IDisposable
{
    ...
    {
        var tokenResponse = await _client.TokenGenerationEndpointAsync(tenantId, request).ConfigureAwait(false);
    ...
    private async Task<(bool Succeeded, TokenResponse? Token)> TryRefreshTokenAsync(RefreshTokenCommand request)
    {
        ...
        try
        {
            var tokenResponse = await _client.RefreshTokenEndpointAsync(tenantKey, request).ConfigureAwait(false);
