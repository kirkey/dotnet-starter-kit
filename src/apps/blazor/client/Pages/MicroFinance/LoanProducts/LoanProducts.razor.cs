...
public partial class LoanProducts
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        _canActivate = await AuthService.HasPermissionAsync(state.User, FshActions.Update, FshResources.LoanProducts);
    }
    ...
    /// <summary>
    /// Activate a loan product using update command.
    /// </summary>
    private async Task ActivateProduct(DefaultIdType id)
    {
        var confirmed = await DialogService.ShowMessageBox(
            "Activate Loan Product",
            "Are you sure you want to activate this loan product? It will be available for new loan applications.",
            yesText: "Activate",
            cancelText: "Cancel");

        if (confirmed == true)
        {
            var product = await Client.GetLoanProductAsync("1", id);
            if (product != null)
            {
                var updateCommand = product.Adapt<UpdateLoanProductCommand>();
                updateCommand.IsActive = true;
                await ApiHelper.ExecuteCallGuardedAsync(
                    () => Client.UpdateLoanProductAsync("1", id, updateCommand),
                    Snackbar,
                    successMessage: "Loan product activated successfully.");
    ...
    /// <summary>
    /// Deactivate a loan product using update command.
    /// </summary>
    private async Task DeactivateProduct(DefaultIdType id)
    {
        var confirmed = await DialogService.ShowMessageBox(
            "Deactivate Loan Product",
            "Are you sure you want to deactivate this loan product? It will no longer be available for new loans.",
            yesText: "Deactivate",
            cancelText: "Cancel");
        ...
        if (confirmed == true)
        {
            ...
            if (product != null)
            {
                ...
                await ApiHelper.ExecuteCallGuardedAsync(
                    () => Client.UpdateLoanProductAsync("1", id, updateCommand),
                    Snackbar,
                    successMessage: "Loan product deactivated successfully.");
                await _table.ReloadDataAsync();
            }
        }
    }
