...
public sealed class UpdateBudgetHandler(IRepository<CostCenter> repository, ILogger<UpdateBudgetHandler> logger)
    : IRequestHandler<UpdateBudgetCommand, DefaultIdType>
{
    private readonly IRepository<CostCenter> _repository = repository ?? throw new ArgumentNullException(nameof(repository));
    private readonly ILogger<UpdateBudgetHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

    public async Task<DefaultIdType> Handle(UpdateBudgetCommand request, CancellationToken cancellationToken)
    {
