...
/// <summary>
/// Excel (.xlsx) parser for Item import using ClosedXML.
/// </summary>
internal sealed class ClosedXmlItemImportParser : IItemImportParser
{
    public async Task<IReadOnlyList<ItemImportRow>> ParseAsync(FileUploadCommand file, CancellationToken cancellationToken)
    {
        ...
        if (!file.IsValid()) return Array.Empty<ItemImportRow>();
        ...
        if (ws is null) return Array.Empty<ItemImportRow>();

        var rows = new List<ItemImportRow>(capacity: 128);
