...
/// <summary>
/// Validator for UpdateProductCommand with comprehensive validation rules.
/// Validates name length, uniqueness (excluding current record), price range, brand existence, and enforces business rules.
/// </summary>
        ...
        [FromKeyedServices("catalog:brands")] IReadRepository<Brand> brandRepository)
    {
        ...
        RuleFor(p => p.Name)
            ...
            .MustAsync(async (command, name, ct) =>
            {
                ...
                return existingProduct is null || existingProduct.Id == command.Id;
            })
