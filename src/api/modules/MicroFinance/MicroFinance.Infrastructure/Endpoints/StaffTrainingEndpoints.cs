...
using FSH.Starter.WebApi.MicroFinance.Application.StaffTrainings.Search.v1;
...
public class StaffTrainingEndpoints : CarterModule
{
    ...
    private const string SearchStaffTrainings = "SearchStaffTrainings";
    ...
    public override void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/search", async (SearchStaffTrainingsCommand command, ISender sender) =>
        {
            var result = await sender.Send(command).ConfigureAwait(false);
            return Results.Ok(result);
        })
        .WithName(SearchStaffTrainings)
        .WithSummary("Search staff trainings")
        .Produces<PagedList<StaffTrainingSummaryResponse>>()
        .RequirePermission(FshPermission.NameFor(FshActions.View, FshResources.MicroFinance))
        .MapToApiVersion(1);
